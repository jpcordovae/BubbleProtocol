...
{
public:
	enum enStatus { CONNECTED, CONNECTING, DISCONNECTED, CLOSING };
	...
	tcp_client(boost::asio::io_service &_io_service): m_io_service(_io_service), socket_(_io_service)
	{
		set_connection_status(DISCONNECTED);
	}
	...
	void write(const char msg) // pass the write data to the do_write function via the io service in the other thread
	{
		...
		#endif
		if(get_connection_status()!=CONNECTED) return;
	...
	void close() // call the do_close function via the io service in the other thread
	{
		...
		#endif
		set_connection_status(CLOSING);
		m_io_service.post(boost::bind(&tcp_client::do_close, this));
	}
  
	enStatus get_connection_status(void)
	{
...
private:
	...
	void do_connect(const char *host, const char *port)
	{
		...
		#endif
		if(get_connection_status()==CONNECTED || get_connection_status()==CONNECTING)
		{
	...
	void set_connection_status(enStatus _status)
	{
		boost::mutex::scoped_lock l(status_mutex);
		status = _status;
	}
	...
	{ // the asynchronous read operation has now completed or failed and returned an error
		...
		{ // write completed, so send next write data
			write_msgs_.pop_front(); // remove the aended data
